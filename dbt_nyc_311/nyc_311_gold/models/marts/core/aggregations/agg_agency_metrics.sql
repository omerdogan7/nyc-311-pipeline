...
-- ========================================
/*
-- 1. Check data quality impact
SELECT 
    agency_name,
    SUM(closed_complaints) as total_closed,
    SUM(valid_response_count) as valid_responses,
    SUM(closed_without_valid_time) as invalid_responses,
...
-- 2. Check SLA compliance (only valid data)
SELECT 
    agency_name,
    AVG(sla_compliance_rate) as avg_sla_rate,
    AVG(total_valid_closed_for_sla) as avg_valid_closed,
...
-- 3. Check response time metrics (only valid)
SELECT 
    agency_name,
    AVG(avg_response_time_hours) as avg_hours,
    AVG(median_response_time_hours) as median_hours,
    AVG(valid_response_count) as avg_valid_count
FROM {{ this }}
WHERE date_key >= 20240101
...
✓ Backlog uses is_closed flag (not status)
*/