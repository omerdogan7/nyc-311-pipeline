...
-- ========================================
/*
-- 1. Hotspot locations with data quality (last 7 days)
SELECT 
    borough,
    city,
    zip_code,
    COUNT(DISTINCT date_key) as days_as_hotspot,
    AVG(total_complaints) as avg_complaints,
    AVG(valid_response_count) as avg_valid_responses,
    AVG(pct_closed_missing_valid_time) as avg_pct_invalid,
    AVG(pct_of_borough_avg) as avg_pct_of_borough,
    MAX(hotspot_severity) as max_severity
FROM {{ this }}
WHERE date_key >= CAST(date_format(date_sub(current_date(), 7), 'yyyyMMdd') AS int)
  AND is_hotspot = true
GROUP BY borough, city, zip_code
ORDER BY days_as_hotspot DESC, avg_complaints DESC
LIMIT 20;

-- 2. Borough comparison with data quality (latest date)
SELECT 
    borough,
    COUNT(DISTINCT location_key) as locations,
    SUM(total_complaints) as total_complaints,
    SUM(closed_complaints) as total_closed,
    SUM(valid_response_count) as total_valid_responses,
    ROUND(SUM(valid_response_count) * 100.0 / NULLIF(SUM(closed_complaints), 0), 2) as valid_data_pct,
    AVG(sla_compliance_rate) as avg_sla_rate,
    SUM(CASE WHEN is_hotspot THEN 1 ELSE 0 END) as hotspot_count
FROM {{ this }}
WHERE date_key = (SELECT MAX(date_key) FROM {{ this }})
GROUP BY borough
ORDER BY total_complaints DESC;

-- 3. Performance metrics by location (only valid data)
SELECT 
    borough,
    city,
    zip_code,
...
WHERE date_key >= CAST(date_format(date_sub(current_date(), 30), 'yyyyMMdd') AS int)
  AND total_valid_closed_for_sla > 0
GROUP BY borough, city, zip_code
HAVING AVG(valid_response_count) >= 10
ORDER BY avg_sla_rate DESC
LIMIT 20;

-- 4. Data quality by borough (last 30 days)
SELECT 
    borough,
    SUM(total_complaints) as total_complaints,
    SUM(closed_complaints) as total_closed,
    SUM(valid_response_count) as total_valid,
    SUM(closed_without_valid_time) as total_invalid,
    ROUND(SUM(valid_response_count) * 100.0 / NULLIF(SUM(closed_complaints), 0), 2) as valid_pct
FROM {{ this }}
WHERE date_key >= CAST(date_format(date_sub(current_date(), 30), 'yyyyMMdd') AS int)
GROUP BY borough
ORDER BY total_complaints DESC;

-- 5. Trending locations with valid data check
SELECT 
    borough,
    city,
    zip_code,
    AVG(complaints_vs_prev_week_pct) as avg_week_over_week_change,
    COUNT(CASE WHEN trend_direction = 'INCREASING' THEN 1 END) as days_increasing,
    AVG(total_complaints) as avg_complaints,
    AVG(pct_closed_missing_valid_time) as avg_invalid_pct
FROM {{ this }}
WHERE date_key >= CAST(date_format(date_sub(current_date(), 30), 'yyyyMMdd') AS int)
GROUP BY borough, city, zip_code
HAVING COUNT(CASE WHEN trend_direction = 'INCREASING' THEN 1 END) >= 15
ORDER BY avg_week_over_week_change DESC
LIMIT 20;

-- 6. Top performing locations (high completion, low invalid data)
SELECT 
    citywide_rank,
    borough,
    city,
...
FROM {{ this }}
WHERE date_key = (SELECT MAX(date_key) FROM {{ this }})
  AND citywide_rank <= 50
  AND total_valid_closed_for_sla > 0
ORDER BY sla_compliance_rate DESC, pct_closed_missing_valid_time ASC
LIMIT 20;

-- 7. Data quality summary by date
SELECT 
    date_key,
    SUM(total_complaints) as total,
    SUM(closed_complaints) as closed,
    SUM(valid_response_count) as valid,
    SUM(closed_without_valid_time) as invalid,
    ROUND(SUM(valid_response_count) * 100.0 / NULLIF(SUM(closed_complaints), 0), 2) as valid_pct
FROM {{ this }}
WHERE date_key >= CAST(date_format(date_sub(current_date(), 30), 'yyyyMMdd') AS int)
GROUP BY date_key
...
-- 8. Incremental run validation
SELECT 
    MIN(date_key) as earliest_date,
    MAX(date_key) as latest_date,
    COUNT(DISTINCT date_key) as days_processed,
    COUNT(DISTINCT location_key) as locations_processed,
...
KEY OPTIMIZATIONS:
==================
1. Jinja variable for min_date_key (single subquery, compile-time)
2. Extended lookback: 7 + 7 = 14 days (for rolling averages)
3. Partition pruning: created_date_key filter (matches fact partitioning)
4. date_sub() syntax (Databricks best practice)
5. Removed date_add() with negative values

FACT TABLE ALIGNMENT:
=====================
✓ Watermark-based incremental strategy
✓ 7 day lookback + 7 day rolling window
✓ is_closed flag usage (consistent with fact)
...
DATA QUALITY:
=============
✓ Response time calculated only for valid closed cases
✓ SLA category excludes invalid values
✓ Tracking metrics for closed without valid data
✓ All validation queries use date_sub (not date_add with negative)
✓ Composite unique key: ['date_key', 'location_key']

GEOGRAPHIC FEATURES:
===================
✓ Hotspot detection (1.5x borough average)
✓ Hotspot severity levels (HIGH/MEDIUM/LOW/NORMAL)
✓ Borough/city rankings
