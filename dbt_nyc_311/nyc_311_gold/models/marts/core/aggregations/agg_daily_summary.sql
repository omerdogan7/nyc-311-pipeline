...
-- 2. SLA compliance trend (only valid data)
SELECT 
    date_actual,
    total_complaints,
    closed_complaints,
    total_valid_closed_for_sla,
    within_sla_count,
    sla_compliance_rate
FROM {{ this }}
...
-- 3. Rolling averages validation (check if window is correct)
SELECT 
    date_actual,
    total_complaints,
    complaints_7day_avg,
    complaints_30day_avg,
    trend_direction
FROM {{ this }}
WHERE date_key >= CAST(date_format(date_sub(current_date(), 60), 'yyyyMMdd') AS int)
ORDER BY date_key DESC;

-- 4. Incremental run validation
SELECT 
...
✅ KEY OPTIMIZATIONS:
====================
1. Jinja variable for single subquery (2x faster)
2. Extended lookback: 7 + 30 = 37 days (rolling averages için)
3. Partition pruning: Sadece created_date_key (closed_date_key kaldırıldı)
4. closed_metrics: Sadece is_closed check (fact ile tutarlı)
5. date_sub syntax (Databricks best practice)

FACT TABLE ILE UYUMLULUK:
=========================
✅ Watermark-based incremental strategy
✅ 7 günlük lookback + 30 gün rolling window
✅ is_closed flag kullanımı (closed_date_key optional)
